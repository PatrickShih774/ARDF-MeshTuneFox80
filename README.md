...
| GPIO12、GPIO13 | ✅ 2 个 | DIO 模式下未接 flash；板载 LED D4/D5 **已拆除** |
...
| | 13 | `EC11_SW` | I | **编码器按键**，接 GND + 10 kΩ 上拉，**中断驱动** |
| **射频与功放** | 12 | `PA_PWR_PWM` | O | 功放功率 PWM（LEDC）→ 升压模块 FB |
| **系统** | 9 | `KEY_USER` | I | 板载 BOOT 按键，启动后可作普通输入 |

**合计：12 个可用引脚全部用满。**数字扩展需求由 TCA9535 的 9 个空闲位承担。
...
### 4.4 TCA9535 I²C 扩展器（地址 `0x20`，用 7/16 位）
...
**已拆除板载 LED D4（GPIO12）与 D5（GPIO13）**：

| LED | 冲突 |
|-----|------|
| **D5 @ GPIO13** | GPIO13 用作 `EC11_SW` 输入，常态上拉为高 → **D5 持续点亮约 1.3 mA**（MCU 深睡的 260 倍，约 31 mAh/天）|
| **D4 @ GPIO12** | GPIO12 输出功放功率 PWM → D4 闪烁 |

状态指示由 **ST7567 液晶**承担；如需外部指示灯，**挂 TCA9535 空闲位**（不占 GPIO）。
